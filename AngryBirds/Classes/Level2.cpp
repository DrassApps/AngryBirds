...
void Level2::configureView() {
    ...
    auto redBirdFrame = SpriteFrameCache::getInstance()->getSpriteFrameByName("bird_black.png");
    ...
    setSpritePositionAndScale(groundPig1,1,330,70);
    setSpritePositionAndScale(firstFloorPig1,1,325,145);
...
void Level2::setSpritePositionAndScale(Sprite* image, float scale, float x, float y) {
    image->setScale(scale);
    image->setPosition(x,y);
}

// Custom physics for rectangle elements of scene, and add custom density for each one
void Level2::buildRectangleElement(Sprite* image, int size, int maskCollision, PhysicsMaterial physicsMaterial) {
...
void Level2::buildCircleElement(Sprite* image, int radius, int maskCollision, PhysicsMaterial physicsMaterial) {
...
void Level2::buildFlyingBird(bool last, float angle) {
    // Create components
    auto redBirdFrame = SpriteFrameCache::getInstance()->getSpriteFrameByName("bird_black.png");
...
void Level2::createMenu() {
    ...
    auto menuItem1 = MenuItemFont::create("Clasificación", CC_CALLBACK_1(Level2::menuRankingCallback, this));
    auto menuItem2 = MenuItemFont::create("Salir", CC_CALLBACK_1(Level2::menuExitCallback, this));
...
void Level2::menuExitCallback(Ref* pSender) {
    auto scene = LevelsScene::createScene();
    Director::getInstance()->replaceScene(TransitionFade::create(1,scene));
}

void Level2::menuRankingCallback(Ref* pSender) {
    ...
    if (shooted) {
        if (score < 200) {
            if ((3 == object1->getCollisionBitmask() && 2 == object2->getCollisionBitmask()) ||
                ...
                // Left pig contact
                if (score < 200 && pigs[0] == 0) { pigContact(groundPig1,0); }
            ...
            if ((5 == object1->getCollisionBitmask() && 2 == object2->getCollisionBitmask()) ||
                ...
                // Big pig contact
                if (score < 200 && pigs[2] == 0) { pigContact(firstFloorPig1,2); }
                highScoreLabel->setString("Puntuación: "+ to_string(score));
            }
        } else {
...
void Level2::pigContact(Sprite* image, int position) {
