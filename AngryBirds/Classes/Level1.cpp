...
bool Level1::init() {
    ...
    this->addChild(woodElLand2);
    this->addChild(groundPig1);
    this->addChild(groundPig2);
...
void Level1::configureView() {
    ...
    auto redBirdFrame = SpriteFrameCache::getInstance()->getSpriteFrameByName("bird_red.png");
    ...
    auto groundPigFrame = SpriteFrameCache::getInstance()->getSpriteFrameByName("pig_minion.png");
    ...
    woodElLand2 = Sprite::createWithSpriteFrame(woodElPort1Frame);
    groundPig1 = Sprite::createWithSpriteFrame(groundPigFrame);
    groundPig2 = Sprite::createWithSpriteFrame(groundPigFrame);
    ...
    woodElLand1->setScale(1.2, 0.5);
    woodElLand1->setPosition(325,115);
    woodElLand2->setScale(0.5, 0.5);
    woodElLand2->setPosition(325,187);
    ...
    setSpritePositionAndScale(woodElPort3,0.45,310,153);
    setSpritePositionAndScale(woodElPort4,0.45,345,153);
    setSpritePositionAndScale(groundPig1,1,300,70);
    setSpritePositionAndScale(groundPig2,1,360,70);
    setSpritePositionAndScale(firstFloorPig1,1,325,217);
    ...
    buildRectangleElement(woodElLand2, 7, 2, PhysicsMaterial(0.05f, 0.01f, 0.1f));
    
    buildRectangleElement(firstFloorPig1, 5, 5, PhysicsMaterial(1.0f, 1.0f, 0.5f));
    
    buildCircleElement(groundPig1, 15, 3, PhysicsMaterial(1.0f, 1.0f, 1.0f));
    buildCircleElement(groundPig2, 15, 4, PhysicsMaterial(1.0f, 1.0f, 1.0f));
...
void Level1::setSpritePositionAndScale(Sprite* image, float scale, float x, float y) {
    image->setScale(scale);
    image->setPosition(x,y);
}

// Custom physics for rectangle elements of scene, and add custom density for each one
void Level1::buildRectangleElement(Sprite* image, int size, int maskCollision, PhysicsMaterial physicsMaterial) {
...
void Level1::buildCircleElement(Sprite* image, int radius, int maskCollision, PhysicsMaterial physicsMaterial) {
...
void Level1::buildFlyingBird(bool last, float angle) {
    // Create components
    auto redBirdFrame = SpriteFrameCache::getInstance()->getSpriteFrameByName("bird_red.png");
...
void Level1::createMenu() {
    ...
    auto menuItem1 = MenuItemFont::create("Clasificación", CC_CALLBACK_1(Level1::menuRankingCallback, this));
    auto menuItem2 = MenuItemFont::create("Salir", CC_CALLBACK_1(Level1::menuExitCallback, this));
...
void Level1::menuExitCallback(Ref* pSender) {
    auto scene = LevelsScene::createScene();
    Director::getInstance()->replaceScene(TransitionFade::create(1,scene));
}

void Level1::menuRankingCallback(Ref* pSender) {
...
bool Level1::onContactBegin(PhysicsContact& contact){
    ...
    if (shooted) {
        if (score < 300) {
            if ((3 == object1->getCollisionBitmask() && 2 == object2->getCollisionBitmask()) ||
                (2 == object1->getCollisionBitmask() && 3 == object2->getCollisionBitmask())) {
                // Left pig contact
                if (score < 300 && pigs[0] == 0) { pigContact(groundPig1,0); }
                highScoreLabel->setString("Puntuación: "+ to_string(score));
            }
            if ((4 == object1->getCollisionBitmask() && 2 == object2->getCollisionBitmask()) ||
                (2 == object1->getCollisionBitmask() && 4 == object2->getCollisionBitmask())) {
                // Right pig contact
                if (score < 300 && pigs[1] == 0) { pigContact(groundPig2,1); }
                highScoreLabel->setString("Puntuación: "+ to_string(score));
            }
            if ((5 == object1->getCollisionBitmask() && 2 == object2->getCollisionBitmask()) ||
                ...
                // Big pig contact
                if (score < 300 && pigs[2] == 0) { pigContact(firstFloorPig1,2); }
                highScoreLabel->setString("Puntuación: "+ to_string(score));
            }
        } else {
...
void Level1::pigContact(Sprite* image, int position) {
